...
class item extends CI_Controller
{
    ...
    function get_ajax()
    {
        ...
        foreach ($list as $item) {
            ...
            // add html for action
            $row[] = '<a href="' . site_url('item/edit/' . $item->item_id) . '" class="btn btn-info btn-xs"><i class="fa fa-edit"></i> Update</a>
    ...
    public function process()
    {
        ...
        if (isset($_POST['add'])) {
            if ($this->item_m->check_barcode($post['barcode'])->num_rows() > 0) {
                $this->session->set_flashdata('error', "Barcode $post[barcode] is already in use");
                redirect('item/add');
            } else {
